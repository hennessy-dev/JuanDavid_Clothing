...
public class ClienteController : BaseApiController
{
  ...
  return new Pager<ClienteDto>(totalRegistros,entidadP.PageIndex,entidadP.PageSize,entidadP.Search);
  }

  [HttpPost]
  [ProducesResponseType(StatusCodes.Status200OK)]
  [ProducesResponseType(StatusCodes.Status400BadRequest)]
  public async Task<ActionResult<IEnumerable<Cliente>>> Post(Dto ClienteDto)
  {
  var resultado = _mapper.Map<Clase>(ClienteDto);
  ...
  [ProducesResponseType(StatusCodes.Status400BadRequest)]
  public async Task<IActionResult> Update(int id, [FromBody] Dto ClienteDto)
  {
      if (id != unameDto.Id)
      {
          return BadRequest();
